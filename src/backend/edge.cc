...
//    verticies_.resize(num_verticies);      // TODO:: there could be problem，e.g. three null are resized here, following we use edge->addVertex, so before vertex it maybe a null element
...
//    return residual_.transpose() * residual_;   // Compute the residual and sqrt_info has already been multiplied, 这里不要再乘
}
